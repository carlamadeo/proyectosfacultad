...
#-----------------------------------------------------------------------------
#
# vcc_continuus_history -s servername -dd datadir -db dbasename -pr projname 
#                       -pv projver -f filename [-o lfile] 
#
# Purpose:
#          Lists the history of an element
#
# Arguments:
...
#            -f filename   = Name of file to analyze
...
#
# Now to analyze the element
#
if {[catch {exec ccm history [file tail $filename]} errmsg]} {
   puts $lfile "Error analyzing the element"
   puts $lfile "$errmsg"
   puts $lfile "\nThis script must fail\n"
   Stopper
   exit 1
} else {
#DEBUG   puts $lfile "Success getting history for element"
   puts $lfile "HISTORY FOR $filename:\n"
   puts $lfile "$errmsg"   
}
