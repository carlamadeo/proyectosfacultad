...
#-----------------------------------------------------------------------------
#
# vcc_clearcase_rmelem -v VOB_tag -w view_tag -f fname [-force] [-c comment] [-o lfile] 
#
# Purpose:
...
#            -f fname       = file to be removed/delted from ClearCase
#            [-force]       = Supress the confirmation step, default is on
...
proc Usage {} {
   ...
   puts $fhand "\nUsage: $comstring -v VOB_tag -w view_tag -f fname \[-force\] \[-c comment\] \[-o logfile\]\n"
}
...
#
# Check entered value, removing directory if necessary
#
...
#
if {$comment == ""} {
   if {[catch "exec cleartool rmelem $force -nc $fname" errmsg]} {
      puts $lfile "rmelem failed with error:\n$errmsg\n\nThis script must fail"
      exit 1
   }
} else {
   if {[catch "exec cleartool rmelem $force -c \"[subst "$comment"]\" [file tail $fname]" errmsg]} {
      puts $lfile "rmelem failed with error:\n$errmsg\n\nThis script must fail"
      exit 1
   }
}

puts $lfile "\nElement removal successful\n"
exit 0
